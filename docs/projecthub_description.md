...
> you around the room — perception, tracking, and motor control all on-device, no cloud,
...
the centre of the bounding box, and you cannot average the whole box either — a YOLOX box
around a person contains a lot of background between the arms and legs and around the
...
if valid.size < roi_mm.size * self.depth_min_valid_fraction:
    return None          # not enough real depth — don't trust it
...
tempting fallback — estimate range from how big the bounding box is — was deliberately not
...
base — a good way to confirm the hard part works before you involve anything that moves:
...
- Obstacle avoidance while following — right now it will happily drive at a chair
